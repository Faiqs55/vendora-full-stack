...
const page = () => {
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    setLoading(true);
    const res = await adminAuthService.registerUser(userData);
    setResponse(res);    
    if(!res.error){

      let loginRes = await adminAuthService.loginUser({email: userData.email, password: userData.password});      
      setResponse(loginRes);
      dispatch(loginAdmin(loginRes))
      router.push("/admin")
    }
    setLoading(false);
  };
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl md:text-3xl font-semibold">Create Your Online Store!</h2>
        {response?.error && (
        ...
        <div className="flex flex-col gap-2 w-full">
          ...
          <div className="flex flex-col w-full">
            <label htmlFor="">Name Your Store:</label>
            <input
              name="storeName"
              onChange={inputChangeHandler}
              className="w-full outline-none border-2 border-gray-200 rounded-sm px-2 py-1 focus:border-gray-500"
